...
int modP(vector<int> A,int n,int p){
    int ans=0;
    for(int i=0;i<n;i++){
        ans = (ans *A[i]* power(10,n-i-1,p))%p;
        ans=ans%p;
    }
    return ans;
}

//usig carry forward approach
int modPC(vector<int> A,int n,int p){
    int ans=0;
    int exp=1;
    for(int i=n-1;i>=0;i--){
        ans+= (A[i]*exp)%p;
...
vector<int> repeatedNumber(vector<int> &A) {
    ...
    int temp=0;
    int n=A.size();
    int slow=(A[0]==n)?0:A[0];
    int fast=(A[0]==n)?0:A[0];
    do{
        slow=(A[slow]==n)?0:A[slow];
        temp=(A[fast]==n)?0:A[fast];
        fast=(A[temp]==n)?0:A[temp];
    }while(slow != fast);
    fast=(A[0]==n)?0:A[0];
    while(slow != fast){
        slow=(A[slow])==n?0:A[slow];
        fast=(A[fast])==n?0:A[fast];
    }
    ans.push_back(slow);
    cout<<slow<<endl;
    ...
    cout<<(n*(n+1)/2)-sum+slow;
    ans.push_back((n*(n+1)/2)-sum+slow);
    return ans;
}
...
int main(){
    ...
    //powM(x,7,17)==1
    //cout<<powM(25,50,24);
    //First number should be greater than second
    //cout<<gcd(1305,4665)<<endl;
    //cout<<__gcd(4665,6905)<<endl;
    //cout<<gcd(90,108);;
    //cout<<powM(7,996,100);
    //string A="C";
    //cout<<A[0]+'0';
